...
#pf[
  + Фиксируем точку $x in X$. Так как Отображение $lambda alpha.#hh alpha x$ непренывно, множество ${alpha in RR | alpha x in V}$ --- окрестность нуля в $RR$. Оно содержит $1\/r_n$, НСНМ. Тогда для некоторого $n$ имеем
    $
