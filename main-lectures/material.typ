...
#th[
  Пусть $(M,d)$ --- полное непустое метрическое пространство. Рассмотрим такое отображение $f : X -> X$, что для некоторой константы $alpha in (0,1)$ выполняется
  $
    forall x,y in X, #h(1em) d(f(x), f(y)) <= alpha dot d(x, y).
  $
  (в таком случае отображение $f$ называется *сжимающим*). Тогда существует единственная точка $x^* in X$ со свойством $f(x^*) = x^*$.
] <fixed-point>
#pf[
  Рассмотрим точку $x_1 in X$. Определим последовательность ${x_n}$ соотношением
  $
    x_(n+1) = f(x_n).
  $
  ...
  $
  Наконец, если нашлась ещё одна точка $y^* in X$ с тем же свойством, то
  $
