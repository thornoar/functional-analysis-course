...
#pf[
  ...
  По @cont-then-bounded[лемме] семейство $Gamma$ равномерно ограничено. В частности, каждое из множеств $Gamma (x)$ ограничено в $Y$, так как ${x}$ ограничено в $X$. Следовательно, $B = X$.
]
...
#th[
  Пусть $(M,d)$ --- полное непустое метрическое пространство. Рассмотрим такое отображение $f : M -> M$, что для некоторой константы $alpha in (0,1)$ выполняется
  $
    forall x,y in M, #h(1em) d(f(x), f(y)) <= alpha dot d(x, y).
  $
  (в таком случае отображение $f$ называется *сжимающим*). Тогда существует единственная точка $x^* in M$ со свойством $f(x^*) = x^*$.
] <fixed-point>
#pf[
  Рассмотрим точку $x_1 in M$. Определим последовательность ${x_n}$ соотношением
  $
    x_(n+1) = f(x_n).
  $
  Мы покажем, что последовательность ${x_n}$ фундаментальна. Действительно, имеем
  $
    d(x_(n+1), x_n) = d(f(x_n), f(x_(n-1))) <= alpha dot d(x_n, x_(n-1)),
  $
  и следовательно
  $
    &d(x_(n+1), x_n) <= alpha^(n-1) d(x_1, x_2)\
    ==> &d(x_n, x_m) <= sum_(k = m)^(n-1) d(x_(k+1), x_k) <= d(x_1, x_2) dot alpha^(m-1) dot sum_(k = 0)^(n-m-1) alpha^k = alpha^(m-1) dot d(x_1,x_2)/(1-alpha) stretch(->)_(n,m -> oo) 0.
  $
  В силу полноты, последовательность ${x_n}$ имеет предел $x^*$. Однако так как отображение $f$ непрерывно (упражнение), имеем
  $
    f(x^*) = f(lim_(n -> oo) x_n) = lim_(n -> oo) f(x_n) = lim_(n -> oo) x_(n+1) = x^*.
  $
  Наконец, если нашлась ещё одна точка $y^* in M$ с тем же свойством, то
  $
    d(x*, y^*) = d(f(x^*), f(y^*)) <= alpha dot d(x^*, y^*) ==> d(x^*, y^*) = 0 ==> x^* = y^*,
  $
  что доказывает единственность.
]

#def[
  Отображение $f : X -> Y$ между метрическими пространствами называется _липшицевым,_ если существует такая константа $M > 0$, что
  $
    d_Y (f(x), f(y)) <= M dot d_X (x,y)
  $
  для всех $x, y in X$.
]

#exer[
  Докажите, что всякая липшицева функция непрерывна.
]

#cor("теорема Пикарда-Линделёфа")[
  Пусть $D subset RR times RR$ --- замкнутый прямоугольник, $(0, y_0) in D^int$. Рассмотрим функцию $f(t,x) : D -> RR$, которая непрерывна по переменной $t$ и липшицева по переменной $y$. Тогда найдётся такое число $epsilon > 0$, что система
  $
    y' (t) = f(t, y(t)), #h(2em) y(0) = y_0
  $ <system>
  имеет единственное решение $y(t)$ на интервале $[-epsilon, epsilon]$
] <picard-lindelof>
#pf[
  Сначала система (@system) переводится в интегральную форму:
  $
    y(t) = y_0 + integral_0^t f(s, y(s)) #hh d s.
  $
  Далее, можно показать, что оператор $T : C[-epsilon, epsilon] -> C[-epsilon, epsilon]$, определённый формулой
  $
    T(phi)(t) = y_0 + integral_0^t f(s, phi(s)) #hh d s,
  $
  является сжимающим в пространстве $(C[-epsilon, epsilon], norm(dot)_oo)$ для достаточно малых значений $epsilon$. Следовательно, по предыдущей теореме существует требуемая функция, и она единственна.
]
